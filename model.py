...
from transformers import BertModel,GPT2Model,GPT2ForSequenceClassification
...
class MyModel(nn.Module):
    def __init__(self):
        ...
        # 加载 bert 中文预训练模型
        # self.bert = GPT2Model.from_pretrained(self.args.bert_pred)
        # self.bert = GPT2ForSequenceClassification.from_pretrained(self.args.bert_pred)
    ...
    def forward(self, x):
        ...
        x = hidden_out.last_hidden_state[:,0]
        # x = self.fc(x)
        # x = self.activation(x)
        pred = self.linear(x)
        # 返回预测结果
